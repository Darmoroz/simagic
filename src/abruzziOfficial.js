...
(async function () {
  try {
    const { data } = await axios.get(
      `https://abruzzi-official.co.uk/shop/?filter_brand=simagic&et_per_page=-1`
    );
    ...
    const productsInfoSimple = productsSimple.map(product => {
      const model = product
        .querySelector('h2.product-title a')
      ...
      const availability = product.classList.contains('instock')
    ...
    await writeErrorToLog('abruzzi-official.co.uk', error);
  }
